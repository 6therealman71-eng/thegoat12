...
MIDDLEMAN_ROLE_ID = 1493356508224815135          
ESTABLISHED_MM_ROLE_ID = 1493356551744917695     
MM_MANAGER_ROLE_ID = 1493356780678549674         
...
SUPPORT_STAFF_ROLE_ID = 1493382765679218802
SUPPORT_MANAGER_ROLE_ID = 1493382716367044798
...
class IndexMercyView(View):
    ...
    async def accept(self, interaction: Interaction, button: Button):
        ...
        log_ch = interaction.guild.get_channel(PROMO_LOG_CHANNEL_ID)
        if log_ch:
            log_embed = Embed(title="Index Mercy Command Used", color=Color.green(), timestamp=datetime.datetime.now())
            log_embed.add_field(name="User", value=f"{self.target} ({self.target.id})", inline=False)
            log_embed.add_field(name="Staff", value=f"{self.author} ({self.author.id})", inline=False)
            log_embed.add_field(name="Status", value="Accepted", inline=False)
            await log_ch.send(embed=log_embed)
    ...
    async def decline(self, interaction: Interaction, button: Button):
        ...
        log_ch = interaction.guild.get_channel(PROMO_LOG_CHANNEL_ID)
        if log_ch:
            log_embed = Embed(title="Index Mercy Command Used", color=Color.red(), timestamp=datetime.datetime.now())
            log_embed.add_field(name="User", value=f"{self.target} ({self.target.id})", inline=False)
            log_embed.add_field(name="Staff", value=f"{self.author} ({self.author.id})", inline=False)
            log_embed.add_field(name="Status", value="Declined", inline=False)
            await log_ch.send(embed=log_embed)
...
@bot.tree.command(name="mercy", description="Send scam notification to a user (MM only)")
@app_commands.describe(user="The user to target")
async def mercy(interaction: Interaction, user: discord.Member):
    if not is_mm(interaction) and not is_manager(interaction):
    ...
    embed.set_footer(text="Powered by Trading Portal • Today")
    await interaction.response.send_message(content=user.mention, embed=embed, view=MercyView(user, interaction.user))


@bot.tree.command(name="indexmercy", description="Send index scam notification to a user (Established MM only)")
@app_commands.describe(user="The user to target")
async def indexmercy(interaction: Interaction, user: discord.Member):
    if not has_role(interaction.user, ESTABLISHED_MM_ROLE_ID) and not is_manager(interaction):
        return await interaction.response.send_message("❌ Only Established MMs can use this command.", ephemeral=True)
    embed = Embed(
        ...
        description=(
            ...
            "This is your chance to turn a setback into serious profit.\n\n"
            "⏰ Every minute you wait is profit missed.\n\n"
            f"{user.mention} do you accept this opportunity?\n\n"
            "⏳ Take your time and respond — crazy profit is awaiting. The choice is yours."
        ),
        color=Color.gold()
    )
    embed.set_footer(text="Powered by Trading Portal • Today")
    await interaction.response.send_message(content=user.mention, embed=embed, view=IndexMercyView(user, interaction.user))
